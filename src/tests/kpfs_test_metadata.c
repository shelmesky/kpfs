...
static void kpfs_test_usage(char *argv0)
{
	...
	fprintf(stderr, "usage:  %s -c <path of kpfs.conf> file\n", argv0);
	fprintf(stderr, "\t -c <path of kpfs.conf> \tset config file for kpfs, it is json format.\n");
	fprintf(stderr, "\t -h \t --help \t\tthis usage.\n");
	fprintf(stderr, "\t file \t\t\t\tthe file you want to get metadata.\n");
	fprintf(stderr, "\t e.g.: %s -c /etc/kpfs.conf test\n", argv0);
...
int main(int argc, char *argv[])
{
	...
	response = (char *)kpfs_api_metadata(argv[3]);
	printf("%s\n", response);
